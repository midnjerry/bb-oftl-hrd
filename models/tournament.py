...
class TournamentMatchUp(db.Model):
  ...
  def schedule_if_ready(self):
    ...
    #Must use .ancestor to ensure previous write transactions are completed.
    #Otherwise members < 2 and assert will fail.
    members = TournamentMatchUpMembership.all().ancestor(self.parent()).filter("match_up =", self).fetch(2)
  ...
  def email(self):
    ...
    for i in range(2):
      ...
      mail.send_mail(
          sender="tournaments@bb-oftl-hrd.appspotmail.com",
          to=team.coach.user.email(),
          bcc="balderasfam@gmail.com",
...
Please contact %s in the OFL/OFTL forums to arrange your match.
  ...
  def advance(self, winner, loser, match=None):
    ...
    if self.next:
      ...
      # idempotence check
      if winner.active:
        tPoints = (TournamentMembership.all().ancestor(tournament).count() - 1)
        winner.team.tpts += tPoints
        coach_stats = CoachStats.all().ancestor(winner.team.coach).get()
        coach_stats.tpts += tPoints
        race_stats = models.RaceStats.all().filter("race =", winner.team.race).get()
        race_stats.tpts += tPoints
        ...
        put_list.extend([winner.team, leader_standing, coach_stats, coach_standing, race_stats])
  ...
  def schedule(self):
    ...
    for round in range(num_rounds):
      ...
      for seed in range(matches_in_round):
        ...
        print "Writing TournamentMatchUp %s" % (match_up.key().name()) 
        match_up.put()

        print "High Seed: %s" % (high_seed)
        print "Low Seed: %s" % (low_seed)
        for team_seed in (high_seed, low_seed):          
          ...
          if (mm.membership):
            print "Writing TournamentMatchUpMembership %s" % (mm.membership.team.key().name())
          else:
            print "Writing TournamentMatchUpMembership None"
          mm.put()
