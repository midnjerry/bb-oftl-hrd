...
from models import Player, Team, CoachStats, Coach, OFLPickemStats
...
'''
class OFLPickemLeader(db.Model):
  # key_name = title
  display_order = db.IntegerProperty()
  top_text      = db.StringProperty()
  formula       = db.StringProperty()
  ...
  def get_score(self, ofl_pickem_stats, season, week):
    score =  int(self.formula, {
        "week":    ofl_pickem_stats.getWeek(season, week),
        "season":    ofl_pickem_stats.getSeason(season, week),
        "alltime":    ofl_pickem_stats.getAllTime(season, week),
        })
    return score
    
class OFLPickemLeaderStanding(db.Model):
  # key_name = coach_key_name
  # parent = OFLPickemLeader
  score    = db.IntegerProperty()
  oflpickem_stats  = db.ReferenceProperty(OFLPickemStats)
'''