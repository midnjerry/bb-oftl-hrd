...
from models import PlayerLeaderStanding, TeamLeaderStanding, Coach
import os.path
import logging
import views
...
def get_ofl_season_and_week():
  ...
  ofl_info = urlfetch.fetch(
      "http://www.oldworldfootball.com/displayWeek.php")
...
#
  ...
  standings = CoachLeaderStanding.all().filter("coach_stats =", coach_stats).fetch(100)
  db.delete(standings)
...
def is_admin(coach_name):
  return coach_name in ["masher", "Thul", "bob152", "mardaed", "elessar9"]
