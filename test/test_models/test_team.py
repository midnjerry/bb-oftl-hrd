...
class TeamTestCase(unittest.TestCase):
  ...
  def testTeam_InsertEmptyTeam_FetchOne(self):
    coach_key = db.Key.from_path('Coach', 'mardaed')
    Team(parent=coach_key).put()
    self.assertEqual(1, len(Team.all().ancestor(coach_key).fetch(2)))
